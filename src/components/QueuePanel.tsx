  ...
  return (
    <section className="glass rounded-xl p-4">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2">
        {topRooms.map((room) => (
          <article key={room.id} className="rounded-lg border border-line bg-panel2 p-3">
            <p className="truncate text-xs font-bold text-muted">{room.name}</p>
            <p className="mt-2 text-xl font-bold" style={{ color: congestionColor(room.queue) }}>{room.queue}</p>
        ...
        {examRooms.map((room) => (
          <div key={room.id} className="rounded-lg border border-line bg-panel2 px-3 py-2">
            <div className="flex items-center justify-between gap-3">
              ...
              <p className="text-lg font-bold" style={{ color: congestionColor(room.queue) }}>{room.queue}</p>
            </div>
            <div className="mt-2 h-1.5 rounded-full bg-bg">
              <div className="h-1.5 rounded-full" style={{ width: `${Math.min(100, room.queue * 2.3)}%`, background: congestionColor(room.queue) }} />
            </div>
          </div>
        ))}
      </div>
